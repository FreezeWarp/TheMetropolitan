        ...
        while ($row = $this->functionMap('fetchAsArray', $this->queryData)) {
          ...
          if ($index === true) { // If the index is boolean "true", we simply create numbered rows to use. (1,2,3,4,5)
            ...
            if ($group) { // Allow duplicate values.
              switch (count($index)) {
                case 1: $data[$row[$index[0]]][] = $row; break;
                case 2: $data[$row[$index[0]]][$row[$index[1]]][] = $row; break;
                case 3: $data[$row[$index[0]]][$row[$index[1]]][$row[$index[2]]][] = $row; break;
                case 4: $data[$row[$index[0]]][$row[$index[1]]][$row[$index[2]]][$row[$index[3]]][] = $row; break;
                case 5: $data[$row[$index[0]]][$row[$index[1]]][$row[$index[2]]][$row[$index[3]]][$row[$index[4]]][] = $row; break;
              }
            }
