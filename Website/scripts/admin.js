...
function authorField() {
    var input = $('<input name="author[]" type="text" autocomplete="off" />');

    $.ajax({
        url: "/admin.php?action=getAuthors",
        dataType: "json",
        data: {
            authorName: ""
        },
...
$(document).ready(function() {
    ...
    // Article Edit Form
    $('input[name=editArticle]').click(function () {
        $('article').html('<form method="post" action="./edit" class="centred">' + $('article').html() + '</form>');
        $('article header > h2').html($('<input name="title" type="text" style="font-size: 2em;"/>').attr('value', $('article header > h2 > a').html()));
        $('article header > .author a[rel=author]').each(function () {
            $('article header > .author').append(authorField().attr('value', $(this).html()));
        });
        $('article header > .author').append($('<input type="button" name="addAuthor" value="+" />'));
        $('article header > .author a[rel=author]').remove();
        $('.mainContent').html('<textarea name="content" style="width: 100%; height: 600px;">' + $('.mainContent').html() + '</textarea><br /><input type="submit" />');

        $('input[name=addAuthor]').click(function () {
            $(this).before(authorField());
        });
        $(this).remove();
    });

    $('input[name=addArticle]').click(function () {
        $('.article:first').prepend('<form method="post" action="./add" class="centred"><div class="article">'
            + '<article>'
            + '<header>'
            + '<h2><input name="title" type="text" style="font-size: 2em;" /></h2>'
            + '<span class="author">by <input type="button" name="addAuthor" value="+" /></span>'
            + '</header>'
            + '<textarea name="content" style="width: 100%; height: 600px;"></textarea>'
            + '</article>'
            + '</div><br /><input type="submit" value="Add Article" /></form>');

        $('input[name=addAuthor]').click(function () {
            $(this).before(authorField());
        });
        $('input[name=addAuthor]').click();

        $(this).remove();
    });

    $('input[name=editAuthor]').click(function () {
        $('div.authorBig > header > div').html('<form method="post" action="./edit" class="centred">' + $('div.authorBig > header > div').html() + '<br /><input type="submit" value="Edit Author" /></form>');

        ["facebook", "twitter", "linkedin", "email", "website"].forEach(function (service) {
            $('div.authorBig > header h1').after('<span style="display: inline-block;"><span class="imageIcon ' + service + 'Icon"></span><input type="text" name="' + service + '" value="' + ($('a[name=' + service + 'Link]').length ? $('a[name=' + service + 'Link]').attr('data-sourceValue') : '') + '" /></span>')
        });
        $('div.authorBig > header h1').html($('<input name="name" type="text" style="font-size: 1.5em;"/>').attr('value', $('div.authorBig > header h1').html()).css('max-width', '100%'));

        $('ul.authorSocialMedia').remove();
        $(this).remove();
    });
});